...
parms<- c(gw_add = gw_add, vfm = vfm, jrange = jrange, hock =  hock, hockros = hockros,dro = dro, mondro = mondro,
          ...
          k_c=k_c, et_slope=et_slope, et_bias=et_bias)
...
optMonth_init <- ga(type = "real-valued", fitness = function(x)
  WB_Optim_ET(c(gw_add=x[1], vfm=x[2], jrange=x[3], hock=x[4], hockros=x[5], dro=x[6], mondro=x[7], aspect=x[8], slope=x[9], shade.coeff=x[10], SWC.Max=x[11], jtemp=x[12], k_c=x[13], et_slope=x[14], et_bias=x[15]),
  ...
  lower=WB_lower, upper=WB_upper, maxiter=100, maxFitness = 0.9)
...
k_c=optValuesM$k_c; et_slope=optValuesM$et_slope; et_bias=optValuesM$et_bias
...
if(make_plots){
  ...
  print(ggparcoord(data=WBcoeffs, columns=1:15, groupColumn=16, scale="uniminmax") + 
          scale_color_gradient(low = "black", high = "gray90") + nps_theme()) # for red: "darkred" and "#fee5d9"
  dev.off()
  
  # Scatterplots of WB coeffs
  WBcoeffs_long <- reshape(WBcoeffs, varying = names(WBcoeffs)[1:15], v.names = "WBcoeffs", timevar = "Variable", 
                           times = names(WBcoeffs)[1:15], direction = "long")
  ...
  wb_optim <- data.frame(var=c('Groundwater Addition', 'Volume Forcing Multiplier', 'Jennings Temperature Range','Hock','Hock Rain on Snow','Direct Runoff','Mondro','Aspect','Slope','Shade Coefficient','Max Soil Water Content','Crop Coefficient','ET Slope','ET Bias','Jennings Temperature'),
                         value=c(gw_add, vfm, jrange, hock, hockros, dro, mondro, aspect, slope, shade.coeff, SWC.Max, k_c, et_slope, et_bias, jtemp),
